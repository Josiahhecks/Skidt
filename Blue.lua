...
-- Remotes
--------------------------------------------------------------------
local DRIVE_REMOTE = rs:WaitForChild("RemoteEvents")
                   :WaitForChild("Main")
                   :WaitForChild("Drive")
...
local function sendKeyState(isDown)
    -- Most games expect "WDown" when the key is pressed and "WUp" when released
...
button.MouseButton1Click:Connect(function()
    ...
    if running then
        ...
        -- Stop the loop and release the key once
        if heartbeat then
